...
ax_slider_real = plt.axes([0.1, 0.25, 0.8, 0.03])  # [left, bottom, width, height]
ax_slider_img = plt.axes([0.1, 0.15, 0.8, 0.03])

realX, imgY = -0.79, 0.15
...
image_m = draw_julia_set(-2.0, 2.0, -2.0, 2.0, 8000, 8000, 150, complex(realX, imgY))

# Display the initial Julia set as a heatmap
...
# Create the sliders
realNumberSlider = Slider(ax_slider_real, "Real(x)", valmin=-2, valmax=2, valinit=realX)
imaginaryNumberSlider = Slider(ax_slider_img, "Img(y)", valmin=-2, valmax=2, valinit=imgY)

def update(val):
    realX = realNumberSlider.val
    imgY = imaginaryNumberSlider.val
    c = complex(realX, imgY)
    image_m = draw_julia_set(-2.0, 2.0, -2.0, 2.0, 8000, 8000, 150, c)
    im.set_array(image_m)
    fig.canvas.draw_idle()

realNumberSlider.on_changed(update)
imaginaryNumberSlider.on_changed(update)
